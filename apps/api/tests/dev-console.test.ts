...
describe('writes that used to leave no audit row', () => {
  ...
  it('category change, shortlist, saved query and export are recorded with the actor', async () => {
    ...
    expect((await call(selector, 'POST', apiPath(API.queryRestore, { id }))).status).toBe(200)
    expect((await trail('query.restore', id))[0]).toMatchObject({ actor_id: 'user_selector', summary: '审计用方案' })
