...
export async function collectReadings(db: Queryable, day: string, config: CapacityConfig): Promise<CapacityReadings> {
  ...
  const sources = await db.query(
    `WITH bounds AS (
       SELECT ($1::date)::timestamp AT TIME ZONE 'UTC' AS lo, ($1::date + 1)::timestamp AT TIME ZONE 'UTC' AS hi
     ), raw AS (
