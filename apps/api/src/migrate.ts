...
const STATEMENTS = [
  ...
  `ALTER TABLE creators ADD COLUMN IF NOT EXISTS metrics jsonb`,
  `ALTER TABLE creators ADD COLUMN IF NOT EXISTS metrics_window int`,
  `ALTER TABLE creators ADD COLUMN IF NOT EXISTS source text`,
  `ALTER TABLE creators ADD COLUMN IF NOT EXISTS external_id text`,
  `ALTER TABLE creators ADD COLUMN IF NOT EXISTS metrics_fetched_at timestamptz`,
  `ALTER TABLE creators ADD COLUMN IF NOT EXISTS metrics_locked jsonb`,
  ...
  `ALTER TABLE ingest_jobs ADD COLUMN IF NOT EXISTS query jsonb`,
  `ALTER TABLE ingest_jobs ADD COLUMN IF NOT EXISTS source_mode text`,
  `CREATE TABLE IF NOT EXISTS creator_raw (
    id text PRIMARY KEY,
    creator_id text NOT NULL REFERENCES creators(id) ON DELETE CASCADE,
  ...
  `CREATE INDEX IF NOT EXISTS creator_raw_creator_fetched_idx ON creator_raw (creator_id, fetched_at DESC)`,
