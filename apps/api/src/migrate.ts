...
const STATEMENTS = [
  ...
  `ALTER TABLE creators ADD COLUMN IF NOT EXISTS photo_urls text[] NOT NULL DEFAULT '{}'`,
  ...
  `ALTER TABLE creators ADD COLUMN IF NOT EXISTS ingest_snapshot jsonb`,
  `ALTER TABLE creators ADD COLUMN IF NOT EXISTS er numeric`,
  ...
  `ALTER TABLE orgs ADD COLUMN IF NOT EXISTS prefs jsonb NOT NULL DEFAULT '{"currency":"CNY","locale":"zh-CN","defaultSort":"rating"}'::jsonb`,
  `CREATE UNIQUE INDEX IF NOT EXISTS orgs_slug_uidx ON orgs (slug) WHERE slug IS NOT NULL`,
  `ALTER TABLE creators ADD COLUMN IF NOT EXISTS org_id text REFERENCES orgs(id)`,
  `CREATE TABLE IF NOT EXISTS company_rule_packs (
    id text PRIMARY KEY,
  ...
  `CREATE TABLE IF NOT EXISTS company_dataset_rows (
    id text PRIMARY KEY,
    org_id text NOT NULL REFERENCES orgs(id) ON DELETE CASCADE,
    dataset text NOT NULL,
    row_key text NOT NULL,
    payload jsonb NOT NULL,
    UNIQUE (org_id, dataset, row_key)
  )`,
  ...
  `CREATE TABLE IF NOT EXISTS company_creator_attrs (
    org_id text NOT NULL REFERENCES orgs(id) ON DELETE CASCADE,
    creator_id text NOT NULL REFERENCES creators(id) ON DELETE CASCADE,
    advice text,
    followers_band text,
