...
export const en: Locale = {
  ...
  kcs: {
    ...
    metric: {
      ...
      readFanRatio: "Reads from followers",
      ...
      retentionRate: "3s read / completion rate",
      noteCount: "Posts",
    ...
    metricHelp: {
      ...
      readFanRatio: "Share of everyday-post reads that come from followers. Usually between 20% and 40%; described, not ranked. From Pugongying; shown as — if unavailable.",
      activeFanRatio: "Followers active on Xiaohongshu in the past 28 days. From Pugongying; shown as — if unavailable.",
      engagedFanRatio: "Followers who liked, saved or commented on the creator's posts in the past 28 days. From Pugongying; shown as — if unavailable.",
      ...
      engagementRate: "Likes, saves and comments per 100 reads, shares not included. Qiangua and Xinhong reads are estimates. Worked out here or from Pugongying; shown as — if unavailable.",
      retentionRate: "Image posts: share of reads that stay 3 seconds. Video posts: completion rate. Both sit in one column for now, so do not compare image and video creators directly. From Pugongying or partner platforms; shown as — if unavailable.",
      noteCount: "Total public posts published in the period. Provided by Xiaohongshu or partner platforms; shown as — if unavailable.",
      ...
      viralRate: "Viral posts ÷ posts. Unsteady below 5 posts; platforms define viral differently, so do not compare across sources. Worked out here; shown as — if unavailable.",
      ...
      cpv: "Quote ÷ median reads on sponsored posts: the cost of one read. Estimated from list price, service fee not included. Worked out here; shown as — if unavailable.",
      cpe: "Estimated cost of one interaction from the list price, before the campaign runs. The platform service fee (10% standard, 20% performance) is not included, so the real cost is usually higher. Worked out here or from Pugongying; shown as — if unavailable.",
      cpm: "Two definitions: Pugongying gives the cost per 1,000 impressions; Qiangua and this system work out the cost per 1,000 reads. The two can differ twofold for the same creator, so check the source before comparing. Service fee not included. Shown as — if unavailable.",
      collectLikeRatio: "Median saves ÷ median likes, compared only within similar content. Not the same as Qiangua's like-and-save ratio, which is (likes + saves) ÷ followers. Worked out here; shown as — if unavailable.",
      purchaseIntentCommentRatio: "Share of comments asking for a link, price or shade. No platform provides this yet, so it is hidden for now.",
      trafficSearchRatio: "Share of reads originating from search. Provided by Pugongying; shown as — if unavailable.",
      trafficRecommendRatio: "Share of reads originating from Explore recommendation feed. Provided by Pugongying; shown as — if unavailable.",
      trafficFollowRatio: "Share of reads originating from following tab. Provided by Pugongying; shown as — if unavailable.",
      readToFollowerRatio: "Median everyday reads ÷ followers: roughly how many reads per follower. Worked out here; shown as — if unavailable.",
      ...
      coopNoteCount: "Sponsored posts filed through Pugongying in the past 30 days, not an all-time total. From Pugongying; shown as — if unavailable."
    },
    metricGroup: {
      scale: "Scale",
      reach: "Reach",
    ...
    tier: {
      ...
      amateur: "Amateur",
      unknown: "Unknown",
      hint: "Head ≥ 500K · Mid 50K – 500K · Junior 5K – 50K · Amateur 300 – 5K (Qiangua's common split)"
    },
    health: {
      label: "Health grade",
      excellent: "Healthy",
      ...
      abnormal: "Flagged",
      unknown: "Unrated",
      hint: "Set by Xiaohongshu on the 1st of each month, with only two grades: healthy or flagged, based on violations, fake traffic, unreported deals and delivery. Avoid flagged creators. Low activity is marked separately and is not part of the grade.",
      healthy: "Healthy",
      lowActive: "Low activity",
      lowActiveHint: "Few recent posts or interactions. A separate note, not part of the health grade."
    },
    band: {
      legend: "Colour = position among creators already in this library from the same source and tier, not a platform-wide ranking; dot = highlight from the saved search",
      cohort: "Position among {n} {tier} creators from {source} in this library, not platform-wide; under 30 is only a rough guide",
      top10: "Top 10% in this library (same source & tier)",
      top25: "Top 25% in this library (same source & tier)",
      upper: "Upper middle in this library (top 25%–50%)",
      lower: "Lower middle in this library (bottom 25%–50%)",
      bottom: "Bottom 25% in this library (same source & tier)",
      none: "Too few similar creators in this library to rank"
    },
    source: {
      label: "Data sources",
    ...
    query: {
      ...
      op: { gte: "≥", lte: "≤", between: "Between", percentileGte: "Tier ranking ≥" },
      value: "Value",
      from: "From",
      to: "To",
      sort: "Sort",
    ...
    landing: {
      ...
      dataLead: "No arbitrary scoring formulas. All metrics connect directly to Pugongying official sources and partner data platforms, with calculated metrics based purely on straightforward ratios. Comparisons are only among creators already in this library from the same source and tier, not platform-wide; missing values remain '—'.",
      ...
      groups: {
        ...
        conversion: "Saves-to-likes ratio, search traffic share (signals before a campaign, not sales)",
      ...
      tierBody: "Head ≥ 500K, Mid 50K – 500K, Junior 5K – 50K, Amateur 300 – 5K. Rankings are computed among creators in this library from the same source and tier, never across sources; the tiers follow Qiangua's common split.",
      gateTitle: "Health grade is a hard gate",
      gateBody: "Xiaohongshu sets the health grade on the 1st of each month: healthy or flagged. Flagged creators are left out. Low activity is marked on its own and does not count as flagged.",
      ...
      values: [
        { title: "No synthetic composite scores", body: "Display objective platform metrics directly. Good or bad is only a relative position among creators in this library from the same source and tier." },
    ...
    brand: {
      ...
      story: "Collects creator information by settings, standardizes viewing criteria, and compares performance among creators in this library from the same source and tier, connecting Xiaohongshu official and partner platforms.",
