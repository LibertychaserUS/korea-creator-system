...
export const en: Locale = {
  ...
  kcs: {
    ...
    metricHelp: {
      followers: "Cumulative followers as shown by the platform.",
      followerGrowth: "Followers gained inside the window.",
      followerGrowthRate: "Gain ÷ followers at the start of the window.",
      readFanRatio: "Share of reads that come from followers. Lower means stronger public reach.",
      activeFanRatio: "Share of followers active in the last 30 days.",
      engagedFanRatio: "Share of followers who liked, saved or commented in the last 30 days.",
      impressionMedian: "Median impressions per post in the window (organic).",
      readMedian: "Median reads per organic post in the window.",
      interactionMedian: "Median likes + saves + comments per post.",
      likeMedian: "Median likes per post.",
      collectMedian: "Median saves per post. Saves signal purchase intent.",
      commentMedian: "Median comments per post.",
      coopReadMedian: "Median reads on disclosed sponsored posts.",
      coopInteractionMedian: "Median interactions on disclosed sponsored posts.",
      engagementRate: "Median interactions ÷ median reads. Per read, not per follower.",
      retentionRate: "3-second read rate (image) or 5-second play rate (video).",
      noteCount: "Posts published inside the window.",
      viralCount: "Viral posts in the window (≥1,000 likes in 12h or ≥5,000 total).",
      viralRate: "Viral posts ÷ posts.",
      priceImage: "Quote for an image post (CNY).",
      priceVideo: "Quote for a video post (CNY).",
      cpv: "Quote ÷ sponsored median reads.",
      cpe: "Quote ÷ sponsored median interactions. Practitioners accept ≤ 3.",
      cpm: "Quote ÷ median reads × 1000.",
      collectLikeRatio: "Median saves ÷ median likes. More saves than likes means lasting intent.",
      purchaseIntentCommentRatio: "Share of comments asking for a link, price or confirming a purchase.",
      trafficSearchRatio: "Share of reads from search. Higher means long-tail conversion.",
      trafficRecommendRatio: "Share of reads from the feed. Higher means burst potential.",
      trafficFollowRatio: "Share of reads from the following tab.",
      readToFollowerRatio: "Median reads ÷ followers. High reads on few followers means the algorithm is pushing.",
      authenticity: "Share of real followers from vendor sampling.",
      coopNoteCount: "Historical disclosed sponsored posts."
    },
    metricGroup: {
      scale: "Scale",
      reach: "Reach",
    ...
    health: {
      ...
      abnormal: "Abnormal",
      unknown: "Unrated",
      hint: "Updated by Pugongying on the 1st of each month. Abnormal is blocked, normal not recommended."
    },
    band: {
      top10: "Top 10% of tier",
      top25: "Top 25% of tier",
      upper: "Upper half of tier",
      lower: "Lower half of tier",
      bottom: "Bottom 25% of tier",
    ...
    source: {
      ...
      fetchedAt: "Fetched",
      locked: "Snapshot at publish"
    },
    query: {
      title: "Saved queries",
      lead: "A query is a set of conditions: tier, health grade, metric thresholds and a sort. Save it and the whole team reuses it.",
      new: "New query",
      unsaved: "Unsaved query",
      name: "Query name",
      namePlaceholder: "e.g. K-beauty value picks",
      save: "Save query",
      saveAs: "Save as",
      delete: "Delete query",
      run: "Run",
      ...
      removeFilter: "Remove",
      noFilters: "No metric conditions yet — only tier and health filters apply.",
      op: { gte: "≥", lte: "≤", between: "Between", percentileGte: "Tier percentile ≥" },
      value: "Value",
      from: "From",
      to: "To",
      sort: "Sort",
      ...
      anyHealth: "Any",
      matched: "{n} matched",
      noResult: "No creator matches this query",
      version: "v{n}",
      saved: "Saved",
      deleted: "Deleted",
    ...
    ingest: {
      title: "Sources",
      lead: "The system is a parameterised crawler: set the parameters, pull JSON from the platform, normalise it through the transform layer.",
      adapters: "Adapters",
      route: "Route",
      ...
      run: "Start fetch",
      running: "Fetching…",
      done: "Fetch done: {written} written, {skipped} skipped, {failed} failed",
      fixtureNote: "This source has no credentials configured; the job returns sample data to verify the field mapping.",
      jobs: "Fetch history",
      jobSource: "Source",
      jobMode: "Mode",
      jobQuery: "Parameters",
      ...
      jobFailed: "Failed",
      jobTime: "Time",
      noJobs: "No fetch jobs yet",
      rawTitle: "Raw response",
      rawLead: "Platform JSON is stored verbatim so the mapping can be audited.",
      transform: "Transform layer",
      transformLead: "One field-mapping table per source. Change the mapping, not the code."
    },
    landing: {
      ...
      previewMeta: "Released · CPE ascending",
      previewCols: { creator: "Creator", tier: "Tier", cpe: "CPE", fans: "Followers" },
      workspacesEyebrow: "Workspaces",
      workspacesTitle: "Three desks. One waterline.",
      workspacesLead: "Intake builds the records, selectors work the pool, ops watches the jobs. Different views, one dataset.",
      flowEyebrow: "Flow",
      flowTitle: "From a single lead to a list you can hand over.",
      steps: [
        { title: "Intake", body: "Display name, followers, quote, collaboration history — saved as a draft." },
        { title: "Fetch", body: "Set parameters, pull creator JSON from Pugongying / Qiangua / Xinhong, normalise through the transform layer." },
        { title: "Assign", body: "Selectors filter, sort, tick and assign to a project in one move." },
        { title: "Monitor", body: "Queued, failed and retried ingest jobs, all on one screen." }
      ],
      dataEyebrow: "Data",
      dataTitle: "What the platform shows is what we look at.",
      dataLead: "No weighted formula. Every field traces back to Pugongying, Qiangua or Xinhong; derived metrics are plain division. Good or bad only means a percentile inside the same follower tier.",
      sourcesTitle: "Three sources, one transform layer",
      sources: {
        pugongying: { label: "Pugongying · official", body: "Median reads / interactions / impressions, engagement rate, reads from fans, quotes, CPE, health grade, audience profile, sponsored history." },
        qiangua: { label: "Qiangua · vendor", body: "Qiangua index, 90-day viral posts, posting frequency, follower growth, authenticity, estimated quotes and CPE / CPM." },
        xinhong: { label: "Xinhong · vendor", body: "Image / video CPE and CPM from campaign analysis, Xinhong index and traffic structure." }
      },
      ...
      tierBody: "Head > 500K, mid 50K – 500K, junior 5K – 50K, amateur 300 – 5K. Percentiles are computed inside one tier only.",
      gateTitle: "Health grade is the gate",
      gateBody: "An abnormal Pugongying health grade excludes the creator; normal is not recommended. It is never weighted.",
      transformTitle: "Transform layer",
      transformBody: "One field-mapping table per source, raw JSON stored verbatim. Change the mapping, not the code.",
      values: [
        { title: "No invented scores", body: "We show the metrics the platform shows. Good or bad is a percentile inside the same tier." },
        { title: "Publish once", body: "Publishing snapshots the metrics. Nothing on the page can move them." },
        { title: "Doors by role", body: "Intake, select and monitor each have their own door. Permissions live on the server." }
      ],
      ctaTitle: "The tide is already moving.",
      ctaLead: "Sign in with your work email. Your role decides the workspace.",
    ...
    brand: {
      ...
      eyebrow: "Global creator intelligence",
      pillars: "Intake · Select · Monitor",
      loginAside: "One account. Your role decides the workspace.",
    ...
    panel: {
      ...
      regions: "Regions",
      interests: "Interests",
      noAudience: "This source did not provide an audience profile",
      collabBrands: "Collaborated brands",
      chooseImage: "Choose image",
      imageHint: "PNG / JPG, square works best.",
      stepSaveHint: "A display name is all you need to save a draft.",
      learnMore: "Explore the workspaces",
      mobileHint: "Rotate or switch to desktop for the full table.",
      showAll: "Show all",
      clearFilters: "Clear filters",
      signOut: "Sign out",
      kpiHint: "From the current library",
      openRate: "Clean rate",
      today: "Today",
      overview: "Overview",
      quickActions: "Quick actions",
      recentActivity: "Recent activity",
      jobDistribution: "Job status distribution",
      untitled: "Untitled",
      preview: "Preview",
      formHint: "Saving creates the creator key as a draft; the score is locked before publishing.",
      publishHint: "Save a draft first, then publish it into the library.",
      ...
      collabQuestion: "Collaborated before?",
      unknownFollowers: "Followers unknown",
      followersRange: "Followers",
      priceRange: "Quote",
      min: "Min",
      max: "Max",
      collabFilter: "Collaboration",
      collabAny: "Any",
      ...
      sources: "Sources",
      batch: "Batch",
      poolLead: "Published, non-blacklisted creators only. Score and grade come from the rule pack and never re-run on language switch.",
      assignTo: "Assign to \"{name}\"",
      assignHint: "To assign creators, open the library from a project page.",
      picked: "{n} selected",
      back: "Back",
      noAssignments: "Nobody on this project yet. Pick a few from the library.",
      emptyProjects: "No projects yet. Create one first.",
      loading: "The tide is on its way…",
      jobStatus: {
        ok: "Done",
        running: "Running",
        queued: "Queued",
